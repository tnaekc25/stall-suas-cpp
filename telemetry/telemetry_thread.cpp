...
#define NOW_NS std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::system_clock::now().time_since_epoch()).count()
...
int telemRead(TelemBuffer *telem_buffer, SerialHandler *serial) {
	...
	while(true) {
		...
		/* No timeout */
			...
			const ssize_t retsize = serial -> readData();
				...
				if (mavlink_parse_char(MAVLINK_COMM_0, (serial -> getBuffer())[i], &message, &status)) {
					...
					switch (message.msgid) {
						...
						case MAVLINK_MSG_ID_TIMESYNC:
							{
								int64_t time_now = NOW_NS;
								mavlink_msg_timesync_decode(&message, &timesync_st);
								if (timesync_st.tc1 == 0) break;

								int64_t offset_ns = timesync_st.tc1 - (time_now + timesync_st.ts1) / 2;
								if (sync_init) 
									 sync_time_ema += EXP_ALPHA * ((double)offset_ns - sync_time_ema);
